 ...
 * catering@meinefirma.de verschickt.
 *
...
async function handleSubmit() {
  ...
  const payload = {
    ...
    targetMailbox: "catering@meinefirma.de"
  };
  ...
  try {
    ...
    setStatus(statusEl, "Catering-Anfrage wurde an catering@meinefirma.de gesendet.", "ok");
    updateRibbonFromTaskpane("Catering angefragt ✓");
  } catch (err) {
    console.error(err);
...
function updateRibbonFromTaskpane(label) {
    ...
    tabs: [
      {
        ...
        groups: [
          {
            id: "CateringGroup",
            controls: [{ id: "CateringToggleButton", label: label, enabled: true }]
          }
        ]
      }
    ]
  }).catch((err) => console.error("Ribbon-Update aus Task Pane fehlgeschlagen:", err));
}
